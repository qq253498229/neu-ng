...
CONF="${script_dir}/../neutralino.config.json"
...
if [ "$1" != "--test" ]; then
    ...
    echo
    rm -rf "${script_dir}/../dist/${APP_BINARY}"
    (cd ${script_dir}/.. && neu build)
...
for APP_ARCH in "${APP_ARCH_LIST[@]}"; do
    ...
    APP_DST="${script_dir}/../dist/win_${APP_ARCH}"
    ...
    RES="${script_dir}/../dist/${APP_BINARY}/resources.neu"
    EXT="${script_dir}/../dist/${APP_BINARY}/extensions"
    ...
    if [ -e "${script_dir}/postproc-win.sh" ]; then
